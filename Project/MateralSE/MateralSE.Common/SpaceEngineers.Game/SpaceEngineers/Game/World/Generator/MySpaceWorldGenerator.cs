namespace SpaceEngineers.Game.World.Generator
{
    ...
    {
        public static Dictionary<string, Vector3> m_compatStaticAsteroids = new Dictionary<string, Vector3>(0x80);
        ...
        static MySpaceWorldGenerator()
        {
            ...
            MyWorldGenerator.OnAfterGenerate += new ActionRef<MyWorldGenerator.Args>(MySpaceWorldGenerator.MyWorldGenerator_AfterGenerate);
        }
        ...
        private static unsafe void AddPregeneratedCluster(ref MyWorldGenerator.Args args, string name, float scale)
        {
            AddPregeneratedAsteroid(name, scale);
            int* numPtr1 = (int*) ref args.AsteroidAmount;
            numPtr1[0]--;
            for (int i = 0; i < 5; i++)
            {
        ...
        public static unsafe void LoadPregeneratedAsteroids(ref MyWorldGenerator.Args args)
        {
            float scale = Math.Max((float) (args.Scenario.AsteroidClustersOffset / 700f), (float) 1f);
            int* numPtr1 = (int*) ref args.AsteroidAmount;
            numPtr1[0] -= MySession.Static.VoxelMaps.Instances.Count;
            ...
            while ((args.AsteroidAmount > 0) && (list.Count > 0))
            {
                ...
                AddPregeneratedCluster(ref args, "asteroid" + num4, scale);
            }
        }
