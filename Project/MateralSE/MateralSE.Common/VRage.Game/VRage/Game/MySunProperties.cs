    ...
    public struct MySunProperties
    {
        ...
        public MyEnvironmentData EnvironmentData
        {
            get
            {
                ...
                data.EnvironmentLight.SunColorRaw = this.EnvironmentLight.SunColorRaw * this.SunIntensity;
                return data;
            }
        }
        public Vector3 SunRotationAxis
        {
            get
            {
                Vector3 vector = (Math.Abs(Vector3.Dot(this.BaseSunDirectionNormalized, Vector3.Up)) <= 0.95f) ? Vector3.Cross(Vector3.Cross(this.BaseSunDirectionNormalized, Vector3.Up), this.BaseSunDirectionNormalized) : Vector3.Cross(Vector3.Cross(this.BaseSunDirectionNormalized, Vector3.Left), this.BaseSunDirectionNormalized);
                vector.Normalize();
                return vector;
            }
        }
