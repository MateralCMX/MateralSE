namespace VRage.Game.VisualScripting.ScriptBuilder.Nodes
{
    ...
    public class MyVisualSyntaxLocalizationNode : MyVisualSyntaxNode
    {
        ...
        internal override void CollectInputExpressions(List<StatementSyntax> expressions)
        {
            ...
            SyntaxNodeOrToken[] tokenArray1 = new SyntaxNodeOrToken[] { SyntaxFactory.Argument(MySyntaxFactory.Literal(typeof(string).Signature(), this.ObjectBuilder.Context)), SyntaxFactory.Token(SyntaxKind.CommaToken), SyntaxFactory.Argument(MySyntaxFactory.Literal(typeof(string).Signature(), this.ObjectBuilder.MessageId)) };
            InvocationExpressionSyntax initializer = SyntaxFactory.InvocationExpression(SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, SyntaxFactory.ElementAccessExpression(SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, SyntaxFactory.IdentifierName("VRage.Game.Localization.MyLocalization"), SyntaxFactory.IdentifierName("Static"))).WithArgumentList(SyntaxFactory.BracketedArgumentList(SyntaxFactory.SeparatedList<ArgumentSyntax>(tokenArray1))), SyntaxFactory.IdentifierName("ToString")));
