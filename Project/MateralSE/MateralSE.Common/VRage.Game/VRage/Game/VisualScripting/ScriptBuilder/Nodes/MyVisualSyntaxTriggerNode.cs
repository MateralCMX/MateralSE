        ...
        protected internal override void Preprocess(int currentDepth)
        {
            if (!base.Preprocessed)
            {
                ...
                for (int i = 0; i < this.ObjectBuilder.InputNames.Count; i++)
                {
                    ...
                    {
                        object[] objArray1 = new object[] { "TriggerNode is missing an input of ", this.ObjectBuilder.InputNames[i], " . NodeId: ", this.ObjectBuilder.ID };
                        throw new Exception(string.Concat(objArray1));
                    }
                    MyVisualSyntaxNode nodeByID = base.Navigator.GetNodeByID(this.ObjectBuilder.InputIDs[i].NodeID);
