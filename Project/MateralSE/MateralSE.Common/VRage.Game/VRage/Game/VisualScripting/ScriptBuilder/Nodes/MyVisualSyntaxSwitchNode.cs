    ...
    public class MyVisualSyntaxSwitchNode : MyVisualSyntaxNode
    {
        ...
        internal override void CollectSequenceExpressions(List<StatementSyntax> expressions)
        {
            ...
            for (int i = 0; i < this.m_sequenceOutputs.Count; i++)
            {
                ...
                if (node != null)
                {
                    ...
                    list.Add(SyntaxFactory.SwitchSection().WithLabels(SyntaxFactory.SingletonList<SwitchLabelSyntax>(SyntaxFactory.CaseSwitchLabel(MySyntaxFactory.Literal(this.ObjectBuilder.NodeType, this.ObjectBuilder.Options[i].Option)))).WithStatements(SyntaxFactory.List<StatementSyntax>(list2)));
                }
            }
        ...
        protected internal override void Preprocess(int currentDepth)
        {
            if (!base.Preprocessed)
            {
                ...
                this.m_sequenceOutputs.ForEach(node => node.Preprocess(currentDepth + 1));
            }
