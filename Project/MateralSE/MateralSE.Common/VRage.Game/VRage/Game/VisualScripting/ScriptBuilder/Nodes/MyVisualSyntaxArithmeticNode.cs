    ...
    public class MyVisualSyntaxArithmeticNode : MyVisualSyntaxNode
    {
        ...
        protected internal override void Preprocess(int currentDepth)
        {
            if (!base.Preprocessed)
            {
                ...
                while (true)
                {
                    if (num >= this.ObjectBuilder.OutputNodeIDs.Count)
                    {
                    ...
                    if (this.ObjectBuilder.OutputNodeIDs[num].NodeID == -1)
                    {
                        throw new Exception("-1 output in arithmetic node: " + this.ObjectBuilder.ID);
                    }
                    MyVisualSyntaxNode nodeByID = base.Navigator.GetNodeByID(this.ObjectBuilder.OutputNodeIDs[num].NodeID);
