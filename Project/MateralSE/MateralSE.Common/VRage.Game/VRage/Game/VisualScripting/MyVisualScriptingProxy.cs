namespace VRage.Game.VisualScripting
{
    ...
    public static class MyVisualScriptingProxy
    {
                ...
                if (type.IsGenericType)
                {
                    ...
                    if (m_whitelistedMethods.TryGetValue(genericTypeDefinition, out set))
                    {
                        ...
                        foreach (MethodInfo info in set)
                        {
                            ...
                            item = !info.IsDefined(typeof(ExtensionAttribute)) ? type.GetMethod(info.Name) : info.MakeGenericMethod(genericArguments);
                            set2.Add(item);
                            m_whitelistedMethodsSequenceDependency[item] = m_whitelistedMethodsSequenceDependency[info];
        ...
        public static bool IsSequenceDependent(this MethodInfo method)
        {
            ...
            bool flag = true;
            return (!m_whitelistedMethodsSequenceDependency.TryGetValue(method, out flag) || flag);
        }
        ...
        public static string MethodGroup(this MethodInfo info)
        {
            ...
            return ((customAttribute == null) ? null : customAttribute.Group);
        }
        ...
        private static void RegisterMethod(Type declaringType, MethodInfo method, VisualScriptingMember attribute, bool? overrideSequenceDependency = new bool?())
        {
            ...
            m_whitelistedMethodsSequenceDependency[method] = (nullable != null) ? nullable.GetValueOrDefault() : attribute.Sequential;
            foreach (KeyValuePair<Type, HashSet<MethodInfo>> pair in m_whitelistedMethods)
            {
                ...
                if (declaringType.IsAssignableFrom(pair.Key))
                {
        ...
        {
            string[] textArray1 = new string[] { info.DeclaringType.Namespace, ".", info.DeclaringType.Name, ".", info.Name };
        ...
        public static string Signature(this Type type) => 
            (!type.IsEnum ? type.FullName : type.FullName.Replace("+", "."));
        ...
        public static IEnumerable<FieldInfo> EventFields =>
            m_visualScriptingEventFields.Values;
