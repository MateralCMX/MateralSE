    ...
    public class MyParticlesManager : MySessionComponentBase
    {
        ...
        public override void Draw()
        {
            if (Enabled)
            {
                ...
                {
                    using (List<MyParticleEffect>.Enumerator enumerator = m_particleEffectsForUpdate.GetEnumerator())
                    {
                        while (enumerator.MoveNext())
                        {
                            enumerator.Current.Draw(m_collectedBillboards);
                        }
                    }
                }
                DrawEnd();
        ...
        protected override void UnloadData()
        {
            ...
            using (List<MyParticleEffect>.Enumerator enumerator = m_effectsToDelete.GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    RemoveParticleEffect(enumerator.Current, false);
                }
            }
        ...
        private static void UpdateEffects()
        {
            ...
            using (List<MyParticleEffect>.Enumerator enumerator = m_effectsToDelete.GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    RemoveParticleEffect(enumerator.Current, true);
                }
            }
            m_effectsToDelete.Clear();
        }
