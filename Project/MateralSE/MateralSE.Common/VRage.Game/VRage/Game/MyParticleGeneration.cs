namespace VRage.Game
{
    ...
    public class MyParticleGeneration : IMyParticleGeneration
    {
        ...
        private static readonly string[] MyBlendTypeStrings = new string[] { 0.ToString(), 1.ToString(), 2.ToString() };
        ...
        private unsafe void ConvertAlphaColors()
        {
            MyAnimatedProperty2DVector4 color = this.Color;
            int index = 0;
            while (index < color.GetKeysCount())
            {
                MyAnimatedPropertyVector4 vector2;
                float num2;
                color.GetKey(index, out num2, out vector2);
                IMyAnimatedProperty property = vector2;
                int num3 = 0;
                while (true)
                {
                    Vector4 vector3;
                    if (num3 >= vector2.GetKeysCount())
                    {
                        index++;
                        break;
                    }
                    vector2.GetKey(num3, out num2, out vector3);
                    vector3 = vector3.UnmultiplyColor();
                    Vector4* vectorPtr1 = (Vector4*) ref vector3;
                    vectorPtr1->W = ColorExtensions.ToLinearRGBComponent(vector3.W);
                    vector3 = Vector4.Clamp(vector3.PremultiplyColor(), new Vector4(0f, 0f, 0f, 0f), new Vector4(1f, 1f, 1f, 1f));
                    property.SetKey(num3, num2, vector3);
                    num3++;
                }
            }
        }
        ...
        private void CreateParticle(Vector3D interpolatedEffectPosition)
        {
            ...
            if (item != null)
            {
                ...
                else
                {
                    ...
                    {
                        randomFloat = MyUtils.GetRandomFloat(1f / randomFloat, randomFloat);
                        vector *= this.m_effect.GetScale() * randomFloat;
                    }
                }
                ...
                {
                    float num7 = item.Velocity.Length();
                    item.Velocity = (Vector3) (MyUtils.Normalize(item.StartPosition - vectord) * num7);
                }
                ...
                {
                    item.Angle = new Vector3(MyUtils.GetRandomFloat(item.Angle.X - angleVar.X, item.Angle.X + angleVar.X), MyUtils.GetRandomFloat(item.Angle.Y - angleVar.Y, item.Angle.Y + angleVar.Y), MyUtils.GetRandomFloat(item.Angle.Z - angleVar.Z, item.Angle.Z + angleVar.Z));
                }
                ...
                this.RadiusVar.GetInterpolatedValue<float>(this.m_effect.GetElapsedTime(), out num2);
                float num3 = 1f;
                if (this.GetEffect().EnableLods)
                {
                    this.LODRadius.GetInterpolatedValue<float>(this.GetEffect().Distance, out num3);
                }
                this.Radius.GetInterpolatedKeys(this.m_effect.GetElapsedTime(), num2, ((this.EnableCustomRadius.GetValue<bool>() ? this.m_effect.UserRadiusMultiplier : 1f) * num3) * this.m_effect.GetScale(), item.Radius);
                ...
                this.Color.GetInterpolatedKeys(this.m_effect.GetElapsedTime(), num4, 1f, item.Color);
                ...
                else
                {
                    ...
                    float randomFloat = MyUtils.GetRandomFloat(1f - this.AccelerationVar, 1f + this.AccelerationVar);
                    this.Acceleration.GetInterpolatedKeys(this.m_effect.GetElapsedTime(), randomFloat, item.Acceleration);
                }
        ...
        public void Deserialize(XmlReader reader)
        {
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    {
                        IMyConstProperty property = properties[index];
        ...
        public void DeserializeFromObjectBuilder(ParticleGeneration generation)
        {
            ...
            if (this.LODBirth.GetKeysCount() > 0)
            {
                float num;
                float num2;
                this.LODBirth.GetKey(this.LODBirth.GetKeysCount() - 1, out num, out num2);
                if (num2 > 0f)
                {
                    this.LODBirth.AddKey<float>(Math.Max((float) (num + 0.25f), (float) 1f), 0f);
                }
            }
        }
        ...
        private void PrepareForDraw()
        {
            ...
            {
                using (List<MyAnimatedParticle>.Enumerator enumerator = this.m_particles.GetEnumerator())
                {
                    while (enumerator.MoveNext())
                    {
                        MyBillboard item = MyTransparentGeometry.AddBillboardParticle(enumerator.Current);
                        if (item != null)
                        {
                            this.m_billboards.Add(item);
                        }
                        MyBillboard billboard1 = item;
                    }
                }
            }
        }
        ...
        private void UpdateParticlesCreation()
        {
            if (this.Enabled.GetValue<bool>() && this.m_show)
            {
                ...
                {
                    float num2;
                    this.MotionInheritance.GetInterpolatedValue<float>(this.m_effect.GetElapsedTime(), out num2);
                    if (num2 > 0f)
                    {
                ...
                if (!this.m_effect.IsEmittingStopped)
                {
                    ...
                    {
                        this.LODBirth.GetInterpolatedValue<float>(this.GetEffect().Distance, out num3);
                    }
                    this.Birth.GetInterpolatedValue<float>(this.m_effect.GetElapsedTime(), out this.m_birthRate);
                    this.m_birthRate *= (0.01666667f * (this.EnableCustomBirth ? this.m_effect.UserBirthMultiplier : 1f)) * num3;
                    if (this.BirthPerFrame.GetKeysCount() > 0)
                    {
                        float num4;
                        float num5;
                        this.BirthPerFrame.GetNextValue(this.m_effect.GetElapsedTime() - 0.01666667f, out this.m_birthPerFrame, out num4, out num5);
                        if ((num4 < (this.m_effect.GetElapsedTime() - 0.01666667f)) || (num4 >= this.m_effect.GetElapsedTime()))
                        {
                            this.m_birthPerFrame = 0f;
                        }
                        else
                        {
                            this.m_birthPerFrame *= (this.EnableCustomBirth ? this.m_effect.UserBirthMultiplier : 1f) * num3;
                        }
                    }
                    this.m_particlesToCreate += this.m_birthRate;
                }
                Vector3 zero = Vector3.Zero;
                if (this.m_lastEffectPosition == null)
                {
                ...
                {
                    zero = (Vector3) ((this.EffectMatrix.Translation - this.m_lastEffectPosition.Value) / ((float) ((int) this.m_particlesToCreate)));
                }
                int num = 40;
                while (true)
                {
                    if (this.m_particlesToCreate >= 1f)
                    {
                        num--;
                        if (num > 0)
                        {
            ...
            while (num < this.m_particles.Count)
            {
                ...
                if (item.Type == MyParticleTypeEnum.Trail)
                {
                    ...
                    else
                    {
                        ...
                        polyLine.LineDirectionNormalized = MyUtils.Normalize((Vector3) (polyLine.Point1 - polyLine.Point0));
                        MyUtils.GetPolyLineQuad(out item.Quad, ref polyLine, MyTransparentGeometry.Camera.Translation);
