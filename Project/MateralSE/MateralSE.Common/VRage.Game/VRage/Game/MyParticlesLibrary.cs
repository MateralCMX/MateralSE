namespace VRage.Game
{
    ...
    public class MyParticlesLibrary
    {
        ...
        public static MyParticleEffect CreateParticleEffect(string name, ref MatrixD effectMatrix, ref Vector3D worldPosition, uint parentID) => 
            (!m_libraryEffectsString.ContainsKey(name) ? null : m_libraryEffectsString[name].CreateInstance(ref effectMatrix, ref worldPosition, parentID));

        public static void DebugDraw()
        {
            using (Dictionary<string, MyParticleEffect>.ValueCollection.Enumerator enumerator = m_libraryEffectsString.Values.GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    MyConcurrentList<MyParticleEffect> instances = enumerator.Current.GetInstances();
                    if (instances != null)
                    {
                        using (ConcurrentEnumerator<FastResourceLockExtensions.MySharedLock, MyParticleEffect, List<MyParticleEffect>.Enumerator> enumerator2 = instances.GetEnumerator())
                        {
        ...
        public static void Deserialize(string file)
        {
            ...
            catch (IOException exception)
            {
                ...
                WinApi.MessageBox(IntPtr.Zero, exception.Message, "Loading Error", 0);
                throw;
            }
        }
