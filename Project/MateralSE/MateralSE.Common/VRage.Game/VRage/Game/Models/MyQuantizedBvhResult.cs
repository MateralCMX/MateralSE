namespace VRage.Game.Models
{
    ...
    public class MyQuantizedBvhResult
    {
        ...
        private float? ProcessTriangle(int triangleIndex)
        {
            ...
            if ((lineTriangleIntersection != null) && float.IsNaN(lineTriangleIntersection.Value))
            {
                MyLog.Default.Warning("Invalid triangle in " + this.m_model.AssetName, Array.Empty<object>());
            }
            if (((lineTriangleIntersection == null) || float.IsNaN(lineTriangleIntersection.Value)) || ((this.m_result != null) && (lineTriangleIntersection.Value >= this.m_result.Value.Distance)))
            {
                return null;
            }
