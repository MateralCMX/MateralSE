namespace VRage.Game.Models
{
    ...
    public static class MyModels
    {
        ...
        {
            MyModel model;
            return ((modelAsset != null) ? (!m_models.TryGetValue(modelAsset, out model) ? null : model) : null);
        }
        ...
        {
            using (List<MyModel>.Enumerator enumerator = GetLoadedModels().GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    enumerator.Current.UnloadData();
                }
            }
            m_models.Clear();
        }
