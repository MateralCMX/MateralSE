namespace VRage.Game.ModAPI.Ingame.Utilities
{
    ...
    public struct StringSegment
    {
        ...
        public bool IsEmpty =>
            ReferenceEquals(this.Text, null);
        public bool IsCached =>
            (this.m_cache != null);
        public int IndexOf(char ch) => 
            ((this.Length != 0) ? this.Text.IndexOf(ch, this.Start, this.Length) : -1);

        public int IndexOf(char ch, int start) => 
            ((this.Length != 0) ? this.Text.IndexOf(ch, this.Start + start, this.Length) : -1);

        public int IndexOfAny(char[] chars) => 
            ((this.Length != 0) ? this.Text.IndexOfAny(chars, this.Start, this.Length) : -1);
