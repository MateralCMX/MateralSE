namespace VRage.Game
{
    ...
    public class MyTransparentGeometry
    {
        ...
        private const int MAX_NEW_PARTICLES_COUNT = 0xaf0;
        private static readonly MyObjectsPool<MyAnimatedParticle> m_animatedParticles = new MyObjectsPool<MyAnimatedParticle>(0xaf0, null);
        ...
        public static void AddLineBillboard(MyStringId material, Vector4 color, Vector3D origin, uint renderObjectID, ref MatrixD worldToLocal, Vector3 directionNormalized, float length, float thickness, MyBillboard.BlendTypeEnum blendType = 0, int customViewProjection = -1, float intensity = 1f, List<MyBillboard> persistentBillboards = null)
        {
            if (IsEnabled)
            {
                ...
                ed.Point0 = origin;
                ed.Point1 = origin + (directionNormalized * length);
                ed.Thickness = thickness;
                if (customViewProjection != -1)
                {
                    translation = MyRenderProxy.BillboardsViewProjectionWrite[customViewProjection].CameraPosition;
                }
                else
                {
                    translation = Camera.Translation;
                }
                Vector3D cameraPosition = translation;
                if (!Vector3D.IsZero(cameraPosition - ed.Point0, 1E-06))
                {
                    MyQuadD dd;
        ...
        public static void AddTriangleBillboard(Vector3D p0, Vector3D p1, Vector3D p2, Vector3 n0, Vector3 n1, Vector3 n2, Vector2 uv0, Vector2 uv1, Vector2 uv2, MyStringId material, uint parentID, Vector3D worldPosition, MyBillboard.BlendTypeEnum blendType = 0)
        {
        ...
        public static void AddTriangleBillboard(Vector3D p0, Vector3D p1, Vector3D p2, Vector3 n0, Vector3 n1, Vector3 n2, Vector2 uv0, Vector2 uv1, Vector2 uv2, MyStringId material, uint parentID, Vector3D worldPosition, Vector4 color, MyBillboard.BlendTypeEnum blendType = 0)
        {
            MyTriangleBillboard billboard;
            MyRenderProxy.TriangleBillboardsPoolWrite.AllocateOrCreate(out billboard);
            billboard.BlendType = blendType;
            ...
            billboard.ColorIntensity = 1f;
            billboard.CustomViewProjection = -1;
            billboard.Reflectivity = MyTransparentMaterials.GetMaterial(material).Reflectivity;
            billboard.ParentID = parentID;
            MyRenderProxy.AddBillboard(billboard);
        }
        ...
        public static void CreateBillboard(MyBillboard billboard, ref MyQuadD quad, MyStringId material, ref Vector4 color, ref Vector3D origin, Vector2 uvOffset, int customViewProjection = -1, float reflectivity = 0f)
        {
            ...
            if (customViewProjection != -1)
            {
                cameraPosition = MyRenderProxy.BillboardsViewProjectionWrite[customViewProjection].CameraPosition;
            }
            else
            {
                cameraPosition = Camera.Translation;
            }
            Vector3D vectord = cameraPosition;
            ...
            {
                billboard.Color *= MathHelper.Clamp((float) ((((float) Math.Sqrt((double) billboard.DistanceSquared)) - material2.AlphaMistingStart) / (material2.AlphaMistingEnd - material2.AlphaMistingStart)), (float) 0f, (float) 1f);
            }
            billboard.Color *= material2.Color;
        }
        ...
        {
            MyLog.Default.WriteLine(string.Format("MyTransparentGeometry.LoadData - START", Array.Empty<object>()));
