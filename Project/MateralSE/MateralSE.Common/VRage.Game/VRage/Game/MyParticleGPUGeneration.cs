namespace VRage.Game
{
    ...
    public class MyParticleGPUGeneration : IComparable, IMyParticleGeneration
    {
        ...
        public void Draw(List<MyBillboard> collectedBillboards)
        {
            ...
            else if (this.m_effect.TransformDirty)
            {
                ...
                MyGPUEmitterTransformUpdate item = new MyGPUEmitterTransformUpdate {
                    ...
                    ParticlesPerFrame = this.GetParticlesPerFrame()
                };
            ...
            else if ((this.ParticlesPerSecond.GetKeysCount() > 1) || (this.ParticlesPerFrame.GetKeysCount() > 0))
            {
                ...
                if ((Math.Abs((float) (this.m_lastFramePPS - particlesPerSecond)) > 0.5f) || (particlesPerFrame > 0f))
                {
                    this.m_lastFramePPS = particlesPerSecond;
                    MyGPUEmitterLite item = new MyGPUEmitterLite {
                        GID = this.m_renderId,
                        ParticlesPerSecond = particlesPerSecond,
                        ParticlesPerFrame = particlesPerFrame
                    };
                    MyParticlesManager.GPUEmittersLite.Add(item);
                }
            }
        }
        ...
        private unsafe void FillData(ref MyGPUEmitter emitter)
        {
            float num;
            float num2;
            MatrixD xd = this.CalculateWorldMatrix();
            ...
            Vector4* vectorPtr1 = (Vector4*) ref emitter.Data.Color0;
            vectorPtr1[0] *= this.m_effect.UserColorMultiplier;
            Vector4* vectorPtr2 = (Vector4*) ref emitter.Data.Color1;
            vectorPtr2[0] *= this.m_effect.UserColorMultiplier;
            Vector4* vectorPtr3 = (Vector4*) ref emitter.Data.Color2;
            vectorPtr3[0] *= this.m_effect.UserColorMultiplier;
            Vector4* vectorPtr4 = (Vector4*) ref emitter.Data.Color3;
            vectorPtr4[0] *= this.m_effect.UserColorMultiplier;
            ...
            float* singlePtr1 = (float*) ref emitter.Data.ParticleSize0;
            singlePtr1[0] *= this.m_effect.UserRadiusMultiplier;
            float* singlePtr2 = (float*) ref emitter.Data.ParticleSize1;
            singlePtr2[0] *= this.m_effect.UserRadiusMultiplier;
            float* singlePtr3 = (float*) ref emitter.Data.ParticleSize2;
            singlePtr3[0] *= this.m_effect.UserRadiusMultiplier;
            float* singlePtr4 = (float*) ref emitter.Data.ParticleSize3;
            singlePtr4[0] *= this.m_effect.UserRadiusMultiplier;
        ...
        private void FillDataComplete(ref MyGPUEmitter emitter)
        {
            ...
            MyRotationReference rotationReference = this.RotationReference;
            flags = (rotationReference == MyRotationReference.Local) ? (flags | GPUEmitterFlags.LocalRotation) : ((rotationReference == MyRotationReference.LocalAndCamera) ? (flags | GPUEmitterFlags.LocalAndCameraRotation) : (flags | (this.Streaks ? GPUEmitterFlags.Streaks : 0)));
            if (this.m_effect.IsSimulationPaused || MyParticlesManager.Paused)
            {
                num2 = 0x80;
            }
            else
            ...
            flags = ((((((((flags | (this.Collide ? GPUEmitterFlags.Collide : 0)) | (this.UseEmissivityChannel ? GPUEmitterFlags.UseEmissivityChannel : 0)) | (this.UseAlphaAnisotropy ? GPUEmitterFlags.UseAlphaAnisotropy : 0)) | (this.SleepState ? GPUEmitterFlags.SleepState : 0)) | (this.Light ? GPUEmitterFlags.Light : 0)) | (this.VolumetricLight ? GPUEmitterFlags.VolumetricLight : 0)) | ((GPUEmitterFlags) num2)) | (MyParticlesManager.Paused ? GPUEmitterFlags.FreezeEmit : 0)) | (this.RotationEnabled ? GPUEmitterFlags.RandomRotationEnabled : 0);
        ...
        private float GetParticlesPerFrame()
        {
            ...
            else
            {
                ...
                {
                    num *= this.m_effect.UserBirthMultiplier;
                }
            }
            return num;
        }
