namespace VRage.Game.Utils
{
    ...
    public class MyCamera : IMyCamera
    {
        ...
        public unsafe void Update(float updateStepTime)
        {
            ...
            {
                Vector3D vectord2;
                Vector3D.Rotate(ref zero, ref viewMatrix, out vectord2);
                MatrixD* xdPtr1 = (MatrixD*) ref viewMatrix;
        ...
        public LineD WorldLineFromScreen(Vector2 screenCoords)
        {
            ...
            Vector4D vectord2 = Vector4D.Transform(vector, matrix);
            Vector4D xyz = Vector4D.Transform(new Vector4D((double) (((2f * screenCoords.X) / this.Viewport.Width) - 1f), (double) (1f - ((2f * screenCoords.Y) / this.Viewport.Height)), 1.0, 1.0), matrix);
            xyz /= xyz.W;
            return new LineD(new Vector3D(vectord2 / vectord2.W), new Vector3D(xyz));
        }
