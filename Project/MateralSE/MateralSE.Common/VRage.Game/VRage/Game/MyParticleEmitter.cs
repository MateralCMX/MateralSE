namespace VRage.Game
{
    ...
    public class MyParticleEmitter
    {
        ...
        public unsafe void CalculateStartPosition(float elapsedTime, MatrixD worldMatrix, Vector3 userAxisScale, float userScale, out Vector3D startOffset, out Vector3D startPosition)
        {
            Vector3 vector;
            Vector3 vector2;
            float num;
            ...
            switch (this.Type)
            {
                ...
                case MyParticleEmitterType.Line:
                    zero = (Vector3.Forward * MyUtils.GetRandomFloat(0f, num)) * vector3;
                    break;
                ...
                case MyParticleEmitterType.Sphere:
                    ...
                    {
                        zero = (MyUtils.GetRandomVector3Normalized() * num) * vector3;
                    }
                    else
                    {
                        float num3;
                        this.LimitAngle.GetInterpolatedValue<float>(elapsedTime, out num3);
                        zero = (MyUtils.GetRandomVector3MaxAngle(MathHelper.ToRadians(num3)) * num) * vector3;
                    }
                    break;

                case MyParticleEmitterType.Hemisphere:
                    zero = (MyUtils.GetRandomVector3HemisphereNormalized(Vector3.Forward) * num) * vector3;
                    break;

                case MyParticleEmitterType.Circle:
                    zero = (MyUtils.GetRandomVector3CircleNormalized() * num) * vector3;
                    break;
            ...
            {
                Matrix matrix = (Matrix.CreateRotationX(MathHelper.ToRadians(vector2.X)) * Matrix.CreateRotationY(MathHelper.ToRadians(vector2.Y))) * Matrix.CreateRotationZ(MathHelper.ToRadians(vector2.Z));
                Vector3* vectorPtr1 = (Vector3*) ref zero;
                Vector3.TransformNormal(ref (Vector3) ref vectorPtr1, ref matrix, out zero);
            }
            if (!this.DirToCamera)
            {
            ...
            else if (MyUtils.IsZero(MyTransparentGeometry.Camera.Forward, 1E-05f))
            {
            ...
            else
            {
                ...
                matrix.Translation = xd2.Translation;
                Vector3D.Transform(ref zero, ref matrix, out vectord2);
