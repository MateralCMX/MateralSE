namespace VRageMath
{
    ...
    public struct BoundingSphere : IEquatable<BoundingSphere>
    {
        ...
        public static BoundingSphere CreateMerged(BoundingSphere original, BoundingSphere additional)
        {
            ...
            sphere.Center = original.Center + (vector2 * (num5 + num4));
            sphere.Radius = num5;
            return sphere;
        }
        ...
        public static void CreateMerged(ref BoundingSphere original, ref BoundingSphere additional, out BoundingSphere result)
        {
            ...
            float num4 = MathHelper.Min(-radius, num - num3);
            float num5 = (float) ((MathHelper.Max(radius, num + num3) - num4) * 0.5);
            result.Center = original.Center + (vector2 * (num5 + num4));
            result.Radius = num5;
        }
        ...
        public static BoundingSphere CreateFromPoints(IEnumerable<Vector3> points)
        {
            ...
            enumerator.MoveNext();
            Vector3 vector6 = vector5 = vector4 = vector3 = vector2 = vector = enumerator.Current;
            foreach (Vector3 vector8 in points)
            {
                ...
                if (vector8.Z > vector.Z)
                {
                    vector = vector8;
                }
            }
            Vector3.Distance(ref vector5, ref vector6, out num);
            Vector3.Distance(ref vector3, ref vector4, out num2);
            Vector3.Distance(ref vector, ref vector2, out num3);
            if (num > num2)
            {
                if (num > num3)
                {
                    Vector3.Lerp(ref vector5, ref vector6, 0.5f, out vector7);
                    num4 = num * 0.5f;
                }
                else
                {
            ...
            {
                Vector3.Lerp(ref vector3, ref vector4, 0.5f, out vector7);
                num4 = num2 * 0.5f;
            }
            else
            {
            ...
            {
                Vector3 vector10;
                vector10.X = vector9.X - vector7.X;
                vector10.Y = vector9.Y - vector7.Y;
                vector10.Z = vector9.Z - vector7.Z;
                float num5 = vector10.Length();
                if (num5 > num4)
                {
                    num4 = (float) ((num4 + num5) * 0.5);
                    vector7 += ((float) (1.0 - (((double) num4) / ((double) num5)))) * vector10;
                }
            }
            sphere.Center = vector7;
            sphere.Radius = num4;
            return sphere;
        }
        ...
        public ContainmentType Contains(BoundingBox box)
        {
            ...
            vector.Y = this.Center.Y - box.Min.Y;
            vector.Z = this.Center.Z - box.Min.Z;
            return ((vector.LengthSquared() > num) ? ContainmentType.Intersects : ContainmentType.Contains);
        }
        ...
        public ContainmentType Contains(Vector3 point) => 
            ((Vector3.DistanceSquared(point, this.Center) >= (this.Radius * this.Radius)) ? ContainmentType.Disjoint : ContainmentType.Contains);
        ...
        public BoundingSphere Translate(ref Vector3 translation) => 
            new BoundingSphere(this.Center + translation, this.Radius);

        public unsafe bool IntersectRaySphere(Ray ray, out float tmin, out float tmax)
        {
            tmin = 0f;
            tmax = 0f;
            ...
            Vector3* vectorPtr1 = (Vector3*) ref v;
            float num3 = vectorPtr1.Dot(v) - (this.Radius * this.Radius);
        ...
        public static BoundingSphere CreateInvalid() => 
            new BoundingSphere(Vector3.Zero, float.MinValue);
    }
}
