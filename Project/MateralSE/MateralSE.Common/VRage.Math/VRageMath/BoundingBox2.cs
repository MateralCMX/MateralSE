namespace VRageMath
{
    ...
    public struct BoundingBox2 : IEquatable<BoundingBox2>
    {
        ...
        public static unsafe BoundingBox2 CreateFromPoints(IEnumerable<Vector2> points)
        {
            if (points == null)
            {
            ...
            bool flag = false;
            Vector2 result = new Vector2(float.MaxValue);
            Vector2 vector2 = new Vector2(float.MinValue);
            foreach (Vector2 vector3 in points)
            {
                Vector2* vectorPtr1 = (Vector2*) ref result;
                Vector2.Min(ref (Vector2) ref vectorPtr1, ref vector3, out result);
                Vector2* vectorPtr2 = (Vector2*) ref vector2;
                Vector2.Max(ref (Vector2) ref vectorPtr2, ref vector3, out vector2);
                flag = true;
            }
            if (!flag)
            {
            ...
            return new BoundingBox2(result, vector2);
        }
        ...
        public Vector2 Center =>
            ((this.Min + this.Max) / 2f);
        public Vector2 HalfExtents =>
            ((this.Max - this.Min) / 2f);
