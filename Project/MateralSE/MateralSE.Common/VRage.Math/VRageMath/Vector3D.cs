namespace VRageMath
{
    ...
    public struct Vector3D : IEquatable<Vector3D>
    {
        ...
        {
            double num1 = a.Dot((Vector3D) b);
            return ((num1 * num1) < ((1E-08 * a.LengthSquared()) * b.LengthSquared()));
        }
        ...
        public static bool TryParse(string str, out Vector3D retval)
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                else if (str[i] == ' ')
                {
                    ...
                    {
                        double result = 0.0;
                        if (!double.TryParse(str.Substring(startIndex, i - startIndex), out result))
                        {
                            flag = false;
                        }
                        if (num3 == 0)
        ...
        public override int GetHashCode() => 
            ((((((int) (this.X * 997.0)) * 0x18d) ^ ((int) (this.Y * 997.0))) * 0x18d) ^ ((int) (this.Z * 997.0)));
        ...
        public long GetHash()
        {
            ...
            num += 0x10000;
            num++;
            return ((((((((((((((long) Math.Round(Math.Abs((double) (this.X * 1000.0)))) * 0x18dL) ^ ((long) Math.Round(Math.Abs((double) (this.Y * 1000.0))))) * 0x18dL) ^ ((long) Math.Round(Math.Abs((double) (this.Z * 1000.0))))) * 0x18dL) ^ (Math.Sign(this.X) + 5)) * 0x18dL) ^ (Math.Sign(this.Y) + 7)) * 0x18dL) ^ (Math.Sign(this.Z) + 11)) * 0x18dL) ^ num);
        }
        ...
        public int AbsMaxComponent() => 
            ((Math.Abs(this.X) <= Math.Abs(this.Y)) ? ((Math.Abs(this.Y) <= Math.Abs(this.Z)) ? 2 : 1) : ((Math.Abs(this.X) <= Math.Abs(this.Z)) ? 2 : 0));

        public static unsafe Vector3D Min(Vector3D value1, Vector3D value2)
        {
            Vector3D vectord;
            Vector3D* vectordPtr1;
            Vector3D* vectordPtr2;
            Vector3D* vectordPtr3;
            vectordPtr1->X = (value1.X < value2.X) ? value1.X : value2.X;
            vectordPtr1 = (Vector3D*) ref vectord;
            vectordPtr2->Y = (value1.Y < value2.Y) ? value1.Y : value2.Y;
            vectordPtr2 = (Vector3D*) ref vectord;
            vectordPtr3->Z = (value1.Z < value2.Z) ? value1.Z : value2.Z;
        ...
        public static unsafe Vector3D Max(Vector3D value1, Vector3D value2)
        {
            Vector3D vectord;
            Vector3D* vectordPtr1;
            Vector3D* vectordPtr2;
        ...
        {
            Vector3D vectord;
            Clamp(ref value1, ref min, ref max, out vectord);
            return vectord;
        }
        ...
        {
            double x = value1.X;
            double num2 = (x > max.X) ? max.X : ((x < min.X) ? min.X : x);
            double y = value1.Y;
            double num4 = (y > max.Y) ? max.Y : ((y < min.Y) ? min.Y : y);
            double z = value1.Z;
            double num6 = (z > max.Z) ? max.Z : ((z < min.Z) ? min.Z : z);
            result.X = num2;
            result.Y = num4;
            result.Z = num6;
        }
        ...
        public static Vector3D ClampToSphere(Vector3D vector, double radius)
        {
            ...
            return ((num <= num2) ? vector : (vector * Math.Sqrt(num2 / num)));
        }
        ...
        public static void Barycentric(Vector3D p, Vector3D a, Vector3D b, Vector3D c, out double u, out double v, out double w)
        {
            ...
            Vector3D vectord1 = p - a;
            double num4 = Dot(vectord1, vectord);
            double num5 = Dot(vectord1, vectord2);
        ...
        {
            Vector3D vectord;
            vectord.X = ((normal.X * matrix.M11) + (normal.Y * matrix.M21)) + (normal.Z * matrix.M31);
            vectord.Y = ((normal.X * matrix.M12) + (normal.Y * matrix.M22)) + (normal.Z * matrix.M32);
            vectord.Z = ((normal.X * matrix.M13) + (normal.Y * matrix.M23)) + (normal.Z * matrix.M33);
            return vectord;
        }
