namespace VRageMath
{
    ...
    public struct MyOrientedBoundingBox : IEquatable<MyOrientedBoundingBox>
    {
        ...
        public static MyOrientedBoundingBox CreateFromBoundingBox(BoundingBox box) => 
            new MyOrientedBoundingBox((box.Min + box.Max) * 0.5f, (box.Max - box.Min) * 0.5f, Quaternion.Identity);
        ...
        public MyOrientedBoundingBox Transform(float scale, Quaternion rotation, Vector3 translation) => 
            new MyOrientedBoundingBox(Vector3.Transform(this.Center * scale, rotation) + translation, this.HalfExtent * scale, this.Orientation * rotation);
        ...
        public unsafe ContainmentType Contains(ref BoundingBox box)
        {
            ...
            matrixPtr1.Translation = Vector3.TransformNormal(((box.Max + box.Min) * 0.5f) - this.Center, matrix);
        ...
        public ContainmentType Contains(ref BoundingSphere sphere)
        {
            ...
            Vector3 vector1 = Vector3.Transform(sphere.Center - this.Center, rotation);
            float num = Math.Abs(vector1.X) - this.HalfExtent.X;
            float num2 = Math.Abs(vector1.Y) - this.HalfExtent.Y;
            float num3 = Math.Abs(vector1.Z) - this.HalfExtent.Z;
        ...
        public bool Intersects(ref BoundingSphere sphere)
        {
            Quaternion rotation = Quaternion.Conjugate(this.Orientation);
            Vector3 vector1 = Vector3.Transform(sphere.Center - this.Center, rotation);
            float num = Math.Max((float) (Math.Abs(vector1.X) - this.HalfExtent.X), (float) 0f);
            float num2 = Math.Max((float) (Math.Abs(vector1.Y) - this.HalfExtent.Y), (float) 0f);
            float num3 = Math.Max((float) (Math.Abs(vector1.Z) - this.HalfExtent.Z), (float) 0f);
            float radius = sphere.Radius;
        ...
        public float? Intersects(ref Ray ray)
        {
            ...
            {
                float num7 = (num3 - this.HalfExtent.Y) / num4;
            ...
            {
                float num9 = (num3 - this.HalfExtent.Z) / num4;
                float num10 = (num3 + this.HalfExtent.Z) / num4;
                if (num9 > num10)
                {
                    num9 = num10;
                    num10 = num9;
        ...
        public float? Intersects(ref Line line)
        {
            if (!this.Contains(ref line.From))
            {
                ...
                float? nullable3 = this.Intersects(ref ray2);
                if (nullable3 != null)
                {
                    ...
                    {
                        return new float?(nullable3.Value);
                    }
                }
                return null;
            }
        ...
        public PlaneIntersectionType Intersects(ref Plane plane)
        {
            ...
            float num2 = (Math.Abs((float) (this.HalfExtent.X * vector.X)) + Math.Abs((float) (this.HalfExtent.Y * vector.Y))) + Math.Abs((float) (this.HalfExtent.Z * vector.Z));
        ...
        {
            Matrix matrix = Matrix.CreateFromQuaternion(this.Orientation);
            Vector3 vector = matrix.Left * this.HalfExtent.X;
            Vector3 vector2 = matrix.Up * this.HalfExtent.Y;
            Vector3 vector3 = matrix.Backward * this.HalfExtent.Z;
            int index = startIndex;
            index++;
            corners[index] = ((this.Center - vector) + vector2) + vector3;
            index++;
            corners[index] = ((this.Center + vector) + vector2) + vector3;
            index++;
            corners[index] = ((this.Center + vector) - vector2) + vector3;
        ...
        public static ContainmentType ContainsRelativeBox(ref Vector3 hA, ref Vector3 hB, ref Matrix mB)
        {
            ...
            Vector3 vector2 = new Vector3(Math.Abs(translation.X), Math.Abs(translation.Y), Math.Abs(translation.Z));
            Vector3 right = mB.Right;
            Vector3 up = mB.Up;
            Vector3 backward = mB.Backward;
            Vector3 vector6 = right * hB.X;
            Vector3 vector7 = up * hB.Y;
            Vector3 vector8 = backward * hB.Z;
            ...
            return ((Math.Abs(Vector3.Dot(translation, vector9)) <= (((Math.Abs((float) (hA.X * vector9.X)) + Math.Abs((float) (hA.Y * vector9.Y))) + Math.Abs(Vector3.Dot(vector9, vector6))) + Math.Abs(Vector3.Dot(vector9, vector7)))) ? ContainmentType.Intersects : ContainmentType.Disjoint);
        }

        public unsafe BoundingFrustum ConvertToFrustum()
        {
            Quaternion quaternion;
            Matrix matrix;
            Quaternion.Conjugate(ref this.Orientation, out quaternion);
            ...
            singlePtr1[0] *= num;
            float* singlePtr2 = (float*) ref matrix.M21;
            singlePtr2[0] *= num;
            float* singlePtr3 = (float*) ref matrix.M31;
            singlePtr3[0] *= num;
            float* singlePtr4 = (float*) ref matrix.M12;
            singlePtr4[0] *= num2;
            float* singlePtr5 = (float*) ref matrix.M22;
            singlePtr5[0] *= num2;
            float* singlePtr6 = (float*) ref matrix.M32;
            singlePtr6[0] *= num2;
            float* singlePtr7 = (float*) ref matrix.M13;
            singlePtr7[0] *= num3;
            float* singlePtr8 = (float*) ref matrix.M23;
            singlePtr8[0] *= num3;
            float* singlePtr9 = (float*) ref matrix.M33;
            singlePtr9[0] *= num3;
            Matrix* matrixPtr1 = (Matrix*) ref matrix;
            matrixPtr1.Translation = (Vector3.UnitZ * 0.5f) + Vector3.TransformNormal(-this.Center, matrix);
