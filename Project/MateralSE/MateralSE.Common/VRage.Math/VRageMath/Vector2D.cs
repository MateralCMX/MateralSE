namespace VRageMath
{
    ...
    public struct Vector2D : IEquatable<Vector2D>
    {
        ...
        static Vector2D()
        {
            ...
            PositiveInfinity = One * (double) 1.0 / (double) 0.0;
        }
        ...
        public static unsafe Vector2D Min(Vector2D value1, Vector2D value2)
        {
            Vector2D vectord;
            Vector2D* vectordPtr1;
            Vector2D* vectordPtr2;
            vectordPtr1->X = (value1.X < value2.X) ? value1.X : value2.X;
            vectordPtr1 = (Vector2D*) ref vectord;
            vectordPtr2->Y = (value1.Y < value2.Y) ? value1.Y : value2.Y;
            vectordPtr2 = (Vector2D*) ref vectord;
            return vectord;
        }
        ...
        public static unsafe Vector2D Max(Vector2D value1, Vector2D value2)
        {
            Vector2D vectord;
            Vector2D* vectordPtr1;
            Vector2D* vectordPtr2;
        ...
        public static Vector2D Clamp(Vector2D value1, Vector2D min, Vector2D max)
        {
            ...
            vectord.X = num3;
            vectord.Y = (num5 < min.Y) ? min.Y : num5;
            return vectord;
        }
        ...
        {
            Vector2D vectord;
            vectord.X = (normal.X * matrix.M11) + (normal.Y * matrix.M21);
            vectord.Y = (normal.X * matrix.M12) + (normal.Y * matrix.M22);
            return vectord;
        }
        ...
        public static Vector2D Transform(Vector2D value, Quaternion rotation)
        {
            ...
            vectord.X = (value.X * ((1.0 - num7) - num8)) + (value.Y * (num6 - num4));
            vectord.Y = (value.X * (num6 + num4)) + (value.Y * ((1.0 - num5) - num8));
            return vectord;
        }
        ...
        public static void Transform(Vector2D[] sourceArray, ref Quaternion rotation, Vector2D[] destinationArray)
        {
            ...
            double num3 = rotation.Z + rotation.Z;
            double num4 = rotation.W * num3;
            double num7 = rotation.Z * num3;
            double num8 = (1.0 - (rotation.Y * num2)) - num7;
            double num1 = rotation.X * num2;
            double num9 = num1 - num4;
            double num10 = num1 + num4;
            double num11 = (1.0 - (rotation.X * num)) - num7;
            for (int i = 0; i < sourceArray.Length; i++)
            {
                double x = sourceArray[i].X;
                double y = sourceArray[i].Y;
                destinationArray[i].X = (x * num8) + (y * num9);
                destinationArray[i].Y = (x * num10) + (y * num11);
            }
        }
        ...
        public static void Transform(Vector2D[] sourceArray, int sourceIndex, ref Quaternion rotation, Vector2D[] destinationArray, int destinationIndex, int length)
        {
            ...
            while (length > 0)
            {
                double x = sourceArray[sourceIndex].X;
                double y = sourceArray[sourceIndex].Y;
                destinationArray[destinationIndex].X = (x * num8) + (y * num9);
                destinationArray[destinationIndex].Y = (x * num10) + (y * num11);
                sourceIndex++;
                destinationIndex++;
                length--;
            }
