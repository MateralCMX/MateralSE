namespace VRageMath
{
    ...
    public struct BoundingBox2D : IEquatable<BoundingBox2D>
    {
        ...
        public static unsafe BoundingBox2D CreateFromPoints(IEnumerable<Vector2D> points)
        {
            if (points == null)
            {
            ...
            bool flag = false;
            Vector2D result = new Vector2D(double.MaxValue);
            Vector2D vectord2 = new Vector2D(double.MinValue);
            foreach (Vector2D vectord3 in points)
            {
                Vector2D* vectordPtr1 = (Vector2D*) ref result;
                Vector2D.Min(ref (Vector2D) ref vectordPtr1, ref vectord3, out result);
                Vector2D* vectordPtr2 = (Vector2D*) ref vectord2;
                Vector2D.Max(ref (Vector2D) ref vectordPtr2, ref vectord3, out vectord2);
                flag = true;
            }
            if (!flag)
            {
            ...
            return new BoundingBox2D(result, vectord2);
        }
        ...
        public unsafe double Perimeter()
        {
            double num;
            Vector2D vectord = this.Max - this.Min;
            Vector2D* vectordPtr1 = (Vector2D*) ref vectord;
            vectordPtr1->X = num = vectord.Y;
            return (2.0 * num);
        }
